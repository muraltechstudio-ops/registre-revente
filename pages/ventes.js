...
export default function VentesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [stockRes, ventesRes] = await Promise.all([
        sb.from('revente_stock').select('*').order('produit'),
        sb
          .from('revente_ventes')
          .select('*')
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const { error } = await getSupabaseClient().from('revente_ventes').insert([
        {
  ...
  const handleDelete = async (id) => {
    ...
    const { error } = await getSupabaseClient().from('revente_ventes').delete().eq('id', id)
    if (error) {
