      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="glass-card rounded-2xl shadow-glass-lg w-full max-w-lg mx-auto p-6"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-serif text-xl font-bold gradient-text">
                {item ? "Modifier l'article" : 'Ajouter un article'}
              </h2>
              <button
                onClick={onClose}
                className="w-8 h-8 flex items-center justify-center rounded-xl text-ink/30 hover:text-ink hover:bg-ink/5 transition-all"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Photo */}
              <div>
                <label className="block text-sm font-medium text-ink/60 mb-1.5 font-sans">
                  Photo <span className="text-ink/30">(optionnelle)</span>
                </label>
                {photoPreview || existingPhoto ? (
                  <div className="relative inline-block">
                    ...
                    <button
                      type="button"
                      onClick={removePhoto}
                      className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-terracotta text-white flex items-center justify-center shadow hover:bg-terracotta-light transition-colors"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ) : (
                  <label className="flex flex-col items-center justify-center w-24 h-24 border-2 border-dashed border-border/60 rounded-xl cursor-pointer hover:border-sage/40 hover:bg-sage/5 transition-all bg-white/40">
                    <Upload className="w-6 h-6 text-ink/20" />
                    <span className="text-[10px] text-ink/20 mt-1 font-sans">Ajouter</span>
              ...
              {/* Catégorie */}
              <div>
                <label className="block text-sm font-medium text-ink/60 mb-1.5 font-sans">Catégorie</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-ink/60 mb-1.5 font-sans">Qté en stock</label>
                  <input
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-ink/60 mb-1.5 font-sans">Prix de revente (€)</label>
                  <input
                    type="number" step="0.01" min="0" required
